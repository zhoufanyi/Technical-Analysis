...
class MarketUniverse:
    ...
    def initializeFromFileName(self,dataFileName):
        ...
        self.MKTUNIV = pd.read_csv('daily/'+dataFileName)
...
#         Strategy.__init__(self,ticker,startdate,enddate )
#         self.startdate
#
...
# class trade:
#     def __init__(self,ticker,signal,init_capital = 10000):
#         self.ticker = ticker
#         self.signal = signal['action']
#         self.int_capital = init_capital
#
#     def tradeRetrun(self):
#         startdate = self.signal.index[0]
#         enddate = self.signal.index[-1]
#         tradeReturn = pd.DataFrame(index =self.signal.index)
#         tradeReturn['Market'] = self.ticker.MKTUNIV['return'].loc[startdate:enddate]
#         tradeReturn['Strategy'] = self.ticker.MKTUNIV['return'].loc[startdate:enddate] * self.signal.shift(1)
#         tradeReturn[['CMarket','CStrategy']] = tradeReturn[['Market','Strategy']].cumsum()
#         tradeReturn[['Market Cash','Strategy Cash']] = tradeReturn[['CMarket','CStrategy']].apply(np.exp)*self.int_capital
#         return tradeReturn
